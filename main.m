...
T_D = 30; % target distance (corresponding time) in ns
...
for ii = 1:length(t)
    sig1(ii) = P_M(t(ii), F_M1, 15, 57);
    sig2(ii) = P_M(t(ii), F_M2, 15, 57);
end
plot(t,sig1); hold on;
plot(t, sig2);
hold off;
...
weight = fastICA(whiteSig,2,1e-5);

S = weight'*sig;
% for ii = 1:2
%     S(ii,:) = S(ii,:).*cos(F_M*t);
%     lowpass(S(ii,:),0.225, 4);
% end
